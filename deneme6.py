...
class HavaSavunmaArayuz(QWidget):
    def __init__(self):
        ...
        self.feedforward_yaw_gain = 0.1
        self.feedforward_pitch_gain = 0.1

        # YENİ EKLEDİM HAREKETLİ SİSTEMDE YUMUŞATMA ANİ DÖNÜŞLER İÇİN
        self.velocity_smoothing_factor = 0.6

        # YENİ: PID'ye giren hata sinyalindeki gürültüyü azaltmak için yumuşatma faktörü.
        # Bu, hem D teriminin hem de ileri beslemenin neden olduğu salınımları engellemek için en önemli adımdır.
        self.error_smoothing_factor = 0.6
        self.smoothed_error_yaw = 0.0
        self.smoothed_error_pitch = 0.0

        self.feedforward_velocity_threshold_px_s = 10.0

        # AYARLANDI: PID Çıkış Limiti (daha hızlı yanıt için daha geniş aralık)
        self.MAX_OUTPUT_DEGREE = 30.0
    ...
    def process_tracking(self, target_x, target_y, frame, target_area_unused, current_frame_time):
        ...
        # 1. Ham (gürültülü) hata değerini derece cinsinden hesapla
        raw_error_yaw_degree = error_yaw_pixel * self.DEGREES_PER_PIXEL_YAW
        raw_error_pitch_degree = error_pitch_pixel * self.DEGREES_PER_PIXEL_PITCH

        # 2. YENİ: Hata sinyalini EMA filtresi ile yumuşat
        alpha = self.error_smoothing_factor
        self.smoothed_error_yaw = (alpha * raw_error_yaw_degree) + (1 - alpha) * self.smoothed_error_yaw
        self.smoothed_error_pitch = (alpha * raw_error_pitch_degree) + (1 - alpha) * self.smoothed_error_pitch
        ...
        # 3. PID hesaplamasının tamamında YUMUŞATILMIŞ hata değerlerini kullan
        self.integral_yaw += self.smoothed_error_yaw * delta_time
        self.integral_yaw = max(min(self.integral_yaw, 20.0), -20.0)

        derivative_yaw = (self.smoothed_error_yaw - self.last_error_yaw) / delta_time if delta_time > 0 else 0
        output_yaw = (self.KP_YAW * self.smoothed_error_yaw +
                      self.KI_YAW * self.integral_yaw +
                      self.KD_YAW * derivative_yaw +
                      feedforward_yaw)
        self.last_error_yaw = self.smoothed_error_yaw

        self.integral_pitch += self.smoothed_error_pitch * delta_time
        self.integral_pitch = max(min(self.integral_pitch, 20.0), -20.0)

        derivative_pitch = (self.smoothed_error_pitch - self.last_error_pitch) / delta_time if delta_time > 0 else 0
        output_pitch = (self.KP_PITCH * self.smoothed_error_pitch +
                        self.KI_PITCH * self.integral_pitch +
                        self.KD_PITCH * derivative_pitch +
                        feedforward_pitch)
        self.last_error_pitch = self.smoothed_error_pitch

        # PID çıktısı için ölü bant
        if abs(self.smoothed_error_yaw) < self.pid_output_deadband_degree:  # Yumuşatılmış hatayı kullan
            output_yaw = 0.0
            self.integral_yaw = 0.0
        if abs(self.smoothed_error_pitch) < self.pid_output_deadband_degree:  # Yumuşatılmış hatayı kullan
            output_pitch = 0.0
            self.integral_pitch = 0.0
