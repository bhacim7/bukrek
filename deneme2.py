...
import os
import traceback
import queue  # Thread güvenli iletişim için

# --- YOLOv11 Model Yapılandırması ---
# DİKKAT: Bu yolu PC'deki best.onnx dosyanızın gerçek yoluyla güncelleyin!
YOLO_MODEL_PATH = "C:/Users/user/yolo11/runs/detect/train2/weights/best.onnx"  # PC yolu
...
def load_yolo_model(model_path):
    ...
    # --- BAŞLANGIÇ: GPU/CPU SEÇİMİ İÇİN DEĞİŞİKLİK ---
    # Bu bayrağı True yaparak CPU kullanımını zorla, False yaparak GPU'yu dene.
    # Eğer çökmeler yaşıyorsanız, bunu True yapmayı deneyin.
    FORCE_CPU_FOR_YOLO = True  # <--- ÇÖKME TESTİ İÇİN BUNU TRUE YAPIN
    ...
    if not FORCE_CPU_FOR_YOLO and 'CUDAExecutionProvider' in ort.get_available_providers():
        ...
        cuda_provider_options = {
            "device_id": 0,  # Kullanılacak GPU ID'si (genellikle 0)
    ...
    providers.append('CPUExecutionProvider')  # Her zaman yedek olarak CPU'yu ekle
    # --- SON: GPU/CPU SEÇİMİ İÇİN DEĞİŞİKLİK ---
    ...
    try:
        if model_path.endswith(".onnx"):
            ...
            print("Desteklenmeyen model formatı. Yalnızca .onnx desteklenir.")
            return None
    except Exception as e:
        print(f"YOLO modeli yüklenirken hata: {e}")
        print(
            "Lütfen YOLO_MODEL_PATH'in doğru olduğundan, gerekli kütüphanelerin (onnxruntime-gpu) yüklü olduğundan ve GPU sürücülerinizin güncel olduğundan emin olun.")
        return None
...
# Eğer yolo_model yüklendiyse, IMG_HEIGHT ve IMG_WIDTH'i modelden al, aksi takdirde varsayılanları kullan
if yolo_model:
    input_name = yolo_model.get_inputs()[0].name
    output_name = yolo_model.get_outputs()[0].name
    input_shape = yolo_model.get_inputs()[0].shape

    # ONNX model giriş şekli (batch, channels, height, width) formatında olmalı
    if len(input_shape) == 4:
        IMG_HEIGHT = input_shape[2]
        IMG_WIDTH = input_shape[3]
        print(f"ONNX model giriş boyutu tespit edildi: {IMG_WIDTH}x{IMG_HEIGHT}")
    else:
        # Eğer input_shape 4 elemanlı değilse veya beklenmedik bir formatta ise
    ...
    # Eğer YOLO modeli yüklenemezse, IMG_HEIGHT ve IMG_WIDTH zaten varsayılan değerlere sahip olacaktır.
    print("YOLO modeli yüklenemedi. Tespitler çalışmayabilir. Varsayılan görüntü boyutları kullanılacak.")
...
class RPiCommunicator(QThread):
    ...
    def __init__(self, rpi_ip, rpi_port):
        ...
        self.socket_buffer = ""  # Gelen verileri tutmak için tampon
    ...
    def run(self):
        ...
        while not self.stop_requested:
            ...
            # Yanıtları dinle (engellemeyen veya kısa süreli engelleyen)
            response = self._receive_response_non_blocking()
            if response:
                self.response_received_signal.emit(response)
                # Eğer bir açı güncellemesi ise, sinyali doğrudan yayınla
                if response.get("action") in ["get_angles", "set_angles", "move_by_direction", "set_proportional_angles_delta"] and response.get("status") == "ok":
    ...
    def _connect_to_rpi(self):
        print(f"DEBUG (RPiComm): RPi'ye bağlanılıyor: {self.rpi_ip}:{self.rpi_port}...")
        try:
            ...
            print(f"DEBUG (RPiComm): Raspberry Pi'ye {self.rpi_ip}:{self.rpi_port} üzerinden başarıyla bağlandı.")
    ...
    def _disconnect_rpi(self):
        print("DEBUG (RPiComm): _disconnect_rpi çağrıldı.")
        if self.rpi_socket and self.is_connected:
            try:
                ...
                print("DEBUG (RPiComm): Raspberry Pi bağlantısı kesildi.")
            except Exception as e:
    ...
    def _send_command(self, command_dict):
        ...
        try:
            ...
            print(f"DEBUG (RPiComm): Komut gönderildi: {message.decode('utf-8').strip()}")
    ...
    def _receive_response_non_blocking(self):
        ...
        try:
            ...
            if not chunk:
                print("DEBUG (RPiComm): _receive_response_non_blocking: Sunucu bağlantıyı kapattı (boş parça).")
                self._disconnect_rpi()
                return None
            ...
            if len(self.socket_buffer) > 4096:
                print("UYARI (RPiComm): Tampon çok büyüdü, '\\n' karakteri bulunamadı. Tampon temizleniyor.")
                self.socket_buffer = ""
                return None
            ...
            traceback.print_exc()
            self.status_update_signal.emit(f"Hata: RPi yanıt hatası: {e}")
            self._disconnect_rpi()
        return None
...
class HavaSavunmaArayuz(QWidget):
    def __init__(self):
        print("DEBUG: HavaSavunmaArayuz başlatıldı.")
        super().__init__()
        self.setWindowTitle('Hava Savunma Sistemi Arayüzü')
        self.setGeometry(100, 100, 1920, 1080)
        self.setStyleSheet("background-color: black;")

        # --- Kullanıcı Arayüzü Elemanlarının Oluşturulması ---
        print("DEBUG: Kullanıcı arayüzü elemanları oluşturuluyor.")
        ...
        print("DEBUG: Kullanıcı arayüzü elemanları oluşturuldu.")

        # KCF ile ilgili değişkenler artık kullanılmıyor veya rolleri değişti
        self.kcf_bbox = None
        self.kcf_active = False  # KCF artık ana izleyici değil
        ...
        # Ateşleme kısıtlı bölge tanımlamaları (şimdi varsayılan değerler, kullanıcı tarafından değiştirilebilir)
        ...
        # AYARLANDI: Nişan alma toleransı (pikseller)
        ...
        self.target_destroyed = False  # Hedefin imha edilip edilmediğini gösterir
        # YENİ: Hedef kaybedildiğinde veya imha edildiğinde yeni hedef aramak için zaman damgası
        self.target_lost_time = 0.0
        # YENİ: Hedef kaybedildiğinde yeni hedef aramadan önce ne kadar bekleneceğini belirten zaman aşımı
        ...
        self.current_tracked_target_class = None  # YENİ: Kilitlenen hedefin sınıfını saklar
        self.current_tracked_target_bbox = None  # YENİ: Kilitlenen hedefin sınırlayıcı kutusunu saklar
        ...
        # AYARLANDI: Daha hızlı ve agresif tepki için PID kazançları artırıldı.
        self.KP_YAW = 0.5  # 0.04'ten 0.06'ya artırıldı
        self.KI_YAW = 0.0002 # 0.0001'den 0.0002'ye artırıldı
        self.KD_YAW = 0.03  # 0.08'den 0.12'ye artırıldı

        self.KP_PITCH = 0.45  # 0.04'ten 0.06'ya artırıldı
        self.KI_PITCH = 0.0002 # 0.0001'den 0.0002'ye artırıldı
        self.KD_PITCH = 0.01  # 0.08'den 0.12'ye artırıldı
        ...
        # YENİ: Mevcut PID kazanç aralığını izlemek için değişken (tek bir set olduğu için sabit kalacak)
        self.current_pid_range = "SINGLE_SET"

        # --- YENİ: Feedforward ve Tahmin Kontrolü için Değişkenler ---
        self.last_target_x = None
        self.last_target_y = None
        self.last_frame_time = None
        self.last_target_velocity_x = 0.0  # Hedefin son bilinen X hızı (piksel/sn)
        self.last_target_velocity_y = 0.0  # Hedefin son bilinen Y hızı (piksel/sn)
        ...
        self.MAX_OUTPUT_DEGREE = 3.0  # 2.0'dan 3.0'a artırıldı, daha büyük düzeltmeler için

        # AYARLANDI: Minimum hareket eşiği (PID çıktısı bu değerden küçükse hareket etmez)
        self.MIN_OUTPUT_DEGREE_THRESHOLD = 0.01  # 0.03'ten 0.01'e düşürüldü, daha ince hareketlere izin vermek için

        # DEGREES_PER_PIXEL_YAW ve DEGREES_PER_PIXEL_PITCH işaretleri kontrol edildi.
        self.DEGREES_PER_PIXEL_YAW = 0.03
        self.DEGREES_PER_PIXEL_PITCH = -0.03

        # AYARLANDI: PID çıkışı için deadband (bu hatanın altındaysa PID çıktısı 0 olur)
        self.pid_output_deadband_degree = 0.01  # 0.02'den 0.01'e düşürüldü, daha hassas kilitlenme için

        # --- Manuel Kontrol için Adım Boyutu ---
        # AYARLANDI: Manuel kontrol için adım boyutu küçültüldü, daha akıcı hareket için.
        ...
        print("DEBUG: RPiCommunicator başlatılıyor.")
        ...
        print("DEBUG: RPiCommunicator başlatıldı.")

        print("DEBUG: Ana düzen ve grup kutuları oluşturuluyor.")
        ...
        self.connect_rpi_button = QPushButton('RPi\'ye Bağlan', self)
        ...
        self.start_button = QPushButton('Kamerayı Başlat', self)
        self.stop_button = QPushButton('Kamerayı Durdur', self)
        ...
        self.fire_weapon_button = QPushButton("ATEŞLE")
        ...
        label_no_fire_start = QLabel("Ateş Etmeme Bölgesi Başlangıç Yaw (°):")
        ...
        label_no_fire_end = QLabel("Ateş Etmeme Bölgesi Bitiş Yaw (°):")
        ...
        self.apply_no_fire_zone_button = QPushButton("Ateş Etmeme Bölgesini Uygula", self)
        ...
        self.clear_no_fire_zone_button = QPushButton("Ateş Etmeme Bölgesini Temizle", self)
        ...
        # --- MANUEL YÖNLENDİRME KONTROL Grup Kutusu ---
        self.direct_manual_control_group_box = QGroupBox("Doğrudan Manuel Yönlendirme")
        ...
        print("DEBUG: Düzen ayarlandı.")

        # Kamera ve Zamanlayıcı Ayarları
        self.capture = None
        self.timer = QTimer(self)
        self.timer.timeout.connect(self.update_frame)
        # Zamanlayıcı, kamera başarıyla başlatıldıktan sonra 'start_camera' içinde başlatılacak.

        self.frame_counter = 0
        self.yolo_ready = yolo_model is not None
        ...
        print("DEBUG: Kamera ve Zamanlayıcı ayarları yapılandırıldı.")

        # Sinyal Bağlantıları
        print("DEBUG: Sinyaller bağlanıyor.")
        ...
        # AYARLANDI: Manuel hareket zamanlayıcısı daha sık tetiklenecek (daha akıcı hareket için)
        self.manual_movement_timer.timeout.connect(self._continuously_update_motor_position)

        # Manuel hareket düğmesi sinyal bağlantıları güncellendi
        ...
        print("DEBUG: Sinyaller bağlandı.")

        QCoreApplication.instance().aboutToQuit.connect(self.close_event)
        print("DEBUG: HavaSavunmaArayuz başlatma tamamlandı.")
    ...
    def _update_status_label(self, message):
        """Tekrarlayan hata mesajlarını önlemek için bekleme süreli slot."""
    ...
    def connect_rpi_threaded(self):
        """RPi'ye ayrı bir iş parçacığında bağlantıyı başlatır."""
    ...
    def _update_rpi_connection_status(self, is_connected):
        """RPi bağlantı durumuna göre UI'yi güncelleyen slot."""
    ...
    def _update_current_angles(self, yaw, pitch):
        """RPiCommunicator'dan alınan mevcut açıları güncelleyen slot."""
        ...
        if response_data.get("status") == "ok":
            if response_data.get("action") == "fire":
                ...
                if self.active_task in ['task1', 'task2', 'task3']:
                    ...
                    self._update_status_label("Durum: Hedef imha edildi. Yeni angajman bekleniyor...")
                    self.target_info_label.setText("Hedef Bilgisi: İmha Edildi.")
                    self.reset_pid_state() # PID durumunu sıfırla
                    print("DEBUG: Ateşlemeden sonra PID ve hedef bilgisi sıfırlandı.")
                ...
                self.reset_pid_state() # PID durumunu sıfırla
                print("DEBUG: Açı sıfırlamadan sonra PID bilgisi sıfırlandı.")
            elif response_data.get("action") in ["set_angles", "move_by_direction", "set_proportional_angles_delta"]:
                pass # Açılar zaten angles_update_signal aracılığıyla güncelleniyor
    ...
    def send_command_to_rpi(self, command_dict):
        """Bir komutu RPiCommunicator'ın kuyruğuna ekler."""
    ...
    def send_proportional_move_command(self, delta_yaw, delta_pitch):
        """
        Taretin belirli orantılı derece miktarları kadar hareket etmesi için Raspberry Pi'ye komut gönderir.
        Bu, PID kontrolü için kullanılır.
        """
        if not self.rpi_thread.is_connected:
            self._update_status_label("Hata: Raspberry Pi'ye bağlı değil, orantılı hareket komutu gönderilemedi.")
            return False
    ...
    def start_camera(self):
        try:
            ...
            for index in camera_indices:
                print(f"Kamera {index} cv2.CAP_DSHOW arka ucu ile deneniyor...")
                try:
                    ...
                    if temp_capture.isOpened():
                        ...
                        print(f"Kamera {index} (cv2.CAP_DSHOW) açılırken başarısız oldu.")
                except Exception as e:
            ...
            print(f"Kamera başarıyla başlatıldı. AYARLANAN Çözünürlük: {actual_width}x{actual_height}")
            ...
            print(f"Kamera başlatılırken beklenmedik bir hata oluştu: {e}")
            traceback.print_exc()
    ...
    def stop_camera(self):
        if self.capture and self.capture.isOpened():
            ...
            print("DEBUG: Kamera durduruldu, tüm PID ve hedef bilgisi sıfırlandı.")
        else:
    ...
    def cancel_task(self):
        ...
        self.is_aimed_at_target = False
        self.reset_pid_state() # PID durumunu sıfırla
        print("DEBUG: Görev iptal edildi, tüm PID ve hedef bilgisi sıfırlandı.")
    ...
    def reset_pid_state(self):
        ...
        self.current_pid_range = "SINGLE_SET"
        print("DEBUG: PID durumu sıfırlandı.")
    ...
    def task1(self):
        ...
        self.is_aimed_at_target = False
        print("DEBUG: Aşama 1 başlatıldı, PID ve hedef bilgisi sıfırlandı.")
    ...
    def task2(self):
        ...
        self.is_target_active = True
        self.is_aimed_at_target = False
        print("DEBUG: Aşama 2 başlatıldı, PID ve hedef bilgisi sıfırlandı.")
    ...
    def task3(self):
        ...
        self.is_target_active = True
        self.is_aimed_at_target = False
        print("DEBUG: Aşama 3 başlatıldı, PID ve hedef bilgisi sıfırlandı.")
    ...
    def set_full_manual_mode(self):
        ...
        try:
            ...
            traceback.print_exc()
            self._update_status_label(f"Hata: Manuel mod başlangıç hatası: {str(e)[:50]}...")
        self.is_target_active = False
        self.is_aimed_at_target = False
        print("DEBUG: Tam Manuel Kontrol Modu başlatıldı, PID ve hedef bilgisi sıfırlandı.")

    def _handle_manual_button_press(self, direction_key):
        """
        Manuel hareket düğmesine basıldığında çağrılır.
        ...
        self._start_manual_movement_timer()
        self._update_status_label(f"Durum: Manuel hareket etkinleştirildi: {direction_key}.")

    def _set_movement_state(self, direction_key, is_pressed):
        """Manuel hareket düğmelerinin basılı olup olmadığını günceller."""
        if self.active_task != 'full_manual':
            return
    ...
    def _continuously_update_motor_position(self):
        ...
        if self.manual_yaw_direction != 0 or self.manual_pitch_direction != 0:
            ...
            # Eğer hareket yönü yoksa, motorları durdurmak için sıfır komutu gönder (önemli!)
    ...
    def _process_yolo_output(self, output, img_width, img_height):
        ...
        for i in range(len(valid_predictions)):
            ...
            height = int(h * img_height / IMG_HEIGHT)
    ...
    def process_yolo_detection(self, frame):
        ...
        if not self.yolo_ready or yolo_model is None or input_name is None or output_name is None:
            print("DEBUG (process_yolo_detection): YOLO modeli hazır değil.")
            return []
        ...
        try:
            ...
            outputs = yolo_model.run([output_name], {input_name: input_image})

            img_height, img_width, _ = frame.shape
            boxes, confidences, class_ids = self._process_yolo_output(outputs[0], img_width, img_height)
            ...
            print(f"HATA (process_yolo_detection): YOLOv11 işleme hatası: {e}")
            traceback.print_exc()
            self._update_status_label(f"Hata: YOLO tespit hatası: {str(e)[:50]}...")
            return []
    ...
    def apply_no_fire_zone_settings(self):
        """Girişlerden ateş etmeme bölgesi değerlerini okur ve uygular."""
        try:
            ...
            self._update_status_label(
                f"Durum: Ateş Etmeme Bölgesi Güncellendi: Yaw [{start_yaw:.1f}°, {end_yaw:.1f}°]")
            print(f"DEBUG: Ateş Etmeme Bölgesi Güncellendi: Yaw [{start_yaw:.1f}°, {end_yaw:.1f}°]")
        except ValueError:
            self._update_status_label("Hata: Lütfen ateş etmeme bölgesi için geçerli sayısal değerler girin.")
        except Exception as e:
            self._update_status_label(f"Hata: Ateş etmeme bölgesi ayarları uygulanırken sorun: {e}")

    def clear_no_fire_zone_settings(self):
        """Ateş etmeme bölgesi ayarlarını temizler."""
        self.no_fire_yaw_start = 0.0
        self.no_fire_yaw_end = 0.0
        self.no_fire_start_input.setText("0.0")
        self.no_fire_end_input.setText("0.0")
        self._update_status_label("Durum: Ateş Etmeme Bölgesi Temizlendi.")
        print("DEBUG: Ateş Etmeme Bölgesi Temizlendi.")

    def send_angle_command(self, yaw, pitch):
        """Taretin belirli bir mutlak açıya gitmesi için Raspberry Pi'ye komut gönderir."""
        if not self.rpi_thread.is_connected:
    ...
    def fire_weapon(self):
        if not self.rpi_thread.is_connected:
            self._update_status_label("Hata: Raspberry Pi'ye bağlı değil, ateşleme yapılamaz.")
            return
        try:
            current_time = time.time()
            ...
            print(f"DEBUG (fire_weapon): Ateşleme denemesi. Mevcut Yaw: {self.current_yaw_angle:.1f}°, "
                  f"Ateş Etmeme Bölgesi: [{self.no_fire_yaw_start:.1f}°, {self.no_fire_yaw_end:.1f}°]")

            if self.is_in_no_fire_zone(self.current_yaw_angle):
                self._update_status_label("Uyarı: Ateş etmeme bölgesindesiniz! Ateşleme engellendi.")
                print("Uyarı: Ateş etmeme bölgesindesiniz! Ateşleme engellendi.")
                return

            if self.active_task == 'full_manual':
                print("Manuel kontrol modunda ateş komutu gönderiliyor...")
            ...
            print("Ateş komutu gönderiliyor...")
            self.send_command_to_rpi({"action": "fire"})
            self.last_fire_time = current_time
        except Exception as e:
    ...
    def reset_rpi_angles(self):
        """Raspberry Pi'ye taret açılarını Yaw ve Pitch 0.0 olarak sıfırlama komutu gönderir."""
        if not self.rpi_thread.is_connected:
    ...
    def close_event(self, event):
        ...
        print("DEBUG: Uygulama kapanıyor (close_event tetiklendi)...")
        self.stop_camera()
    ...
    def update_frame(self):
        display_frame = None
        try:
            print("DEBUG (update_frame): Çerçeve güncelleme döngüsü başlatıldı.")
            ...
            if not ret or frame is None or frame.size == 0:
                print("HATA (update_frame): Çerçeve okunamadı, boş veya geçersiz boyut. Kamera durduruluyor.")
                self._update_status_label("Hata: Kameradan çerçeve okunamadı.")
                self.stop_camera()
                return
            ...
            print("DEBUG (update_frame): YOLO tespiti başlatılıyor.")
            if self.is_target_active and self.yolo_ready:
                detections = self.process_yolo_detection(display_frame)
                print(f"DEBUG (update_frame): YOLO {len(detections)} tespit buldu.")

                if self.current_tracked_target_class is not None and not self.target_destroyed:
                    print(f"DEBUG: Kilitli hedef '{self.current_tracked_target_class}' takip ediliyor.")
                    ...
                    if closest_locked_detection:
                        ...
                        self.target_info_label.setText(
                            f"Hedef: YOLO Takip Ediliyor ({detected_class_status}).")
                        self.target_lost_time = 0.0
                    else:
                        print(
                            f"DEBUG: Kilitli hedef sınıfı için YOLO tespiti bulunamadı ({self.current_tracked_target_class}) veya çok uzakta.")
                        ...
                        if current_frame_time - self.target_lost_time < self.prediction_time_limit:
                            ...
                            self.target_info_label.setText("Hedef: Takip Kaybedildi. Tahminle hareket ediliyor.")
                            ...
                            if self.last_target_x is not None and self.last_frame_time is not None:
                                ...
                                if self.current_tracked_target_bbox:
                                    ...
                                    print(f"DEBUG: Hedef tahmin edildi: X={predicted_x:.1f}, Y={predicted_y:.1f}")
                                else:
                                    current_target_bbox_for_pid = None
                            else:
                                current_target_bbox_for_pid = None
                            ...
                            print(f"DEBUG: Hedef takibi kaybedildi ve zaman aşımı doldu. Hedef kalıcı olarak kaybedildi.")
                            current_target_bbox_for_pid = None
                            self.current_tracked_target_class = None
                            self.current_tracked_target_bbox = None
                            self.target_destroyed = True
                            self.waiting_for_new_engagement_command = True
                            self.target_info_label.setText("Hedef: Takip Kaybedildi. Yeni hedef aranıyor.")
                            self.reset_pid_state()
                            self._update_status_label("Durum: Hedef kalıcı olarak kaybedildi. Yeni hedef aranıyor.")
                            print("DEBUG: Hedef kaybedildi ve zaman aşımı doldu, PID ve hedef bilgisi sıfırlandı.")
                elif self.waiting_for_new_engagement_command or self.current_tracked_target_class is None:
                    print("DEBUG: Yeni hedef edinme/yeniden edinme süreci başlatıldı.")
                    ...
                    if self.active_task == 'task1':
                        ...
                        if candidate_target:
                            self.target_info_label.setText(
                                f"Hedef Bilgisi: {candidate_target['class_name']} tespit edildi.")
                            detected_class_status = candidate_target['class_name']
                            self._update_status_label("Durum: Aşama 1 - Hedef kilitlendi.")
                        else:
                            self.target_info_label.setText("Hedef Bilgisi: Balon tespit edilmedi.")
                        ...
                        for det in red_balloons:
                            ...
                            if distance < minimum_distance:
                                ...
                                candidate_target = det
                        if candidate_target:
                            self.target_info_label.setText(f"Hedef Bilgisi: Kırmızı Balon tespit edildi.")
                                ...
                                if data:
                                    try:
                                        ...
                                        if target_color:
                                            ...
                                            self._update_status_label(
                                                f"Durum: QR Kodu: Hedef {target_color} {target_shape if target_shape else ''}.")

                                        if no_fire_zone and len(no_fire_zone) == 2:
                                            self.no_fire_yaw_start = no_fire_zone[0]
                                            self.no_fire_yaw_end = no_fire_zone[1]
                                            print(
                                                f"DEBUG: Ateş etmeme bölgesi güncellendi: {self.no_fire_yaw_start}-{self.no_fire_yaw_end}")
                                        ...
                                        if move_restricted_zone and len(move_restricted_zone) == 2:
                                            ...
                                            print(
                                                f"DEBUG: Hareket kısıtlı bölgesi güncellendi: {self.movement_restricted_yaw_start}-{self.movement_restricted_yaw_end}")
                        ...
                        if self.active_engagement_target_color:
                            ...
                            if candidate_target:
                                self.target_info_label.setText(
                                    f"Hedef Bilgisi: Angajman Hedefi ({candidate_target['class_name']}) tespit edildi.")
                                detected_class_status = candidate_target['class_name']
                    ...
                    if candidate_target:
                        ...
                        self.reset_pid_state()
                        print(f"DEBUG: Yeni hedef kilitlendi: {self.current_tracked_target_class}. PID sıfırlandı.")
                    else:
                        current_target_bbox_for_pid = None
                else:
                    current_target_bbox_for_pid = None
                    ...
                    if self.target_destroyed and self.waiting_for_new_engagement_command:
                        print("DEBUG: PID sıfırlandı (Hedef İmha Edildi/Bekleniyor).")
                        self.target_info_label.setText("Hedef Bilgisi: İmha Edildi. Yeni angajman bekleniyor.")
                    self.target_lost_time = 0.0

            print("DEBUG (update_frame): YOLO tespitleri çiziliyor.")
            for det in detections:
            ...
            print("DEBUG (update_frame): PID kontrolü başlatılıyor.")
            ...
            print("DEBUG (update_frame): Göreve özel durum güncellemeleri.")
            if self.active_task == 'task1':
                if current_target_bbox_for_pid and not self.target_destroyed:
                    self._update_status_label("Durum: Aşama 1 - Hedefe Nişan Alınıyor (Manuel Ateş).")
                elif self.target_destroyed:
                    self._update_status_label("Durum: Aşama 1 - Hedef imha edildi. Yeni hedef bekleniyor.")
                else:
                ...
                if current_target_bbox_for_pid and detected_class_status == "red_balloon" and self.is_aimed_at_target and not self.target_destroyed:
                    try:
                        self.fire_weapon()
                        self._update_status_label("Durum: Aşama 2 - Düşman imha edildi! (Otonom)")
                        print("DEBUG: Düşman hedef tespit edildi ve ateşlendi (Aşama 2 - Otonom)!")
                    except Exception as e:
                    ...
                    self._update_status_label("Durum: Aşama 2 - Dost hedef tespit edildi, ateşleme engellendi.")
                elif current_target_bbox_for_pid and not self.target_destroyed:
                    self._update_status_label("Durum: Aşama 2 - Düşman hedefe nişan alınıyor...")
                elif self.target_destroyed:
                    self._update_status_label("Durum: Aşama 2 - Hedef imha edildi. Yeni hedef bekleniyor.")
                else:
                ...
                if current_target_bbox_for_pid and detected_class_status == self.active_engagement_target_color and self.is_aimed_at_target and not self.target_destroyed:
                    if not self.is_in_no_fire_zone(self.current_yaw_angle):
                        try:
                            self.fire_weapon()
                            self._update_status_label("Durum: Aşama 3 - Hedef imha edildi! (Otonom)")
                            print("DEBUG: Hedef tespit edildi ve ateşlendi (Aşama 3 - Otonom)!")
                        except Exception as e:
                            print(f"HATA: Aşama 3 otomatik ateşleme sırasında hata: {e}")
                            traceback.print_exc()
                            self._update_status_label(f"Hata: Aşama 3 ateşleme hatası: {str(e)[:50]}...")
                    else:
                        self._update_status_label("Durum: Aşama 3 - Ateş etmeme bölgesinde ateşleme engellendi!")
                elif current_target_bbox_for_pid and detected_class_status != self.active_engagement_target_color:
                    self._update_status_label(
                        "Durum: Aşama 3 - Dost/Yanlış hedef tespit edildi, ateşleme engellendi.")
                elif current_target_bbox_for_pid and not self.target_destroyed:
                    self._update_status_label("Durum: Aşama 3 - Angajman hedefine nişan alınıyor...")
                elif self.target_destroyed:
                    self._update_status_label("Durum: Aşama 3 - Hedef imha edildi. Yeni hedef bekleniyor.")
                else:
                    self._update_status_label("Durum: Aşama 3 - Angajman Hedefi Yok.")
            elif self.active_task == 'full_manual':
                if self.is_in_no_fire_zone(self.current_yaw_angle):
                    self._update_status_label(
                        f"Durum: Tam Manuel Kontrol Modu - Ateş Etmeme Bölgesinde! Yaw: {self.current_yaw_angle:.1f}°, Pitch: {self.current_pitch_angle:.1f}°")
                else:
                    self._update_status_label(
            ...
            print("DEBUG (update_frame): Çerçeve ekranda gösteriliyor.")
            self._display_frame(display_frame)

            self.frame_counter += 1
            print("DEBUG (update_frame): Çerçeve güncelleme döngüsü tamamlandı.")
    ...
    def is_in_no_fire_zone(self, current_yaw_angle):
        """
        Mevcut yaw açısının ateş etmeme bölgesinde olup olmadığını kontrol eder, 0/360 derece etrafındaki dönüşü yönetir.
        Mevcut_yaw_açısı ve bölge girişlerinin derece cinsinden olduğunu varsayar.
        """
        zone_start = self.no_fire_yaw_start
        zone_end = self.no_fire_yaw_end

        print(
            f"DEBUG (is_in_no_fire_zone): current_yaw_angle={current_yaw_angle:.1f}°'nin [{zone_start:.1f}°, {zone_end:.1f}°] bölgesinde olup olmadığı kontrol ediliyor.")
    ...
    def is_in_movement_restricted_zone(self, target_yaw_angle):
        """Aşama 3 için kısıtlı hareket bölgesini kontrol eder, 0/360 derece etrafındaki dönüşü yönetir."""
    ...
    def process_tracking(self, target_x, target_y, frame, target_area_unused, current_frame_time):
        ...
        # Minimum hareket eşiği (PID çıktısı bu eşiğin altındaysa, hareket etmez)
        if 0 < abs(output_yaw) < self.MIN_OUTPUT_DEGREE_THRESHOLD:
            output_yaw = 0.0 # Hareket etmemesi için sıfırla
        if 0 < abs(output_pitch) < self.MIN_OUTPUT_DEGREE_THRESHOLD:
            output_pitch = 0.0 # Hareket etmemesi için sıfırla

        # Çıktıları maksimum derece ile sınırla
        ...
        # Hareket kısıtlı bölgesi kontrolü (Aşama 3 için)
        # Eğer hareket kısıtlı bölgeye giriyorsa, o eksendeki hareketi sıfırla
        if self.active_task == 'task3':
            # Öngörülen yeni açıları hesapla
            ...
            # Pitch için de benzer bir kontrol eklenebilir eğer pitch kısıtlamaları varsa
            # if self.is_in_movement_restricted_zone_pitch(predicted_pitch_after_move):
            #     output_pitch = 0.0

        # Yalnızca sıfır olmayan bir hareket varsa komut gönder
        if output_yaw != 0.0 or output_pitch != 0.0:
            print(
                f"PID Hata Ayıklama (Orantılı): Hata Yaw (px): {error_yaw_pixel}, Hata Yaw (°): {error_yaw_degree:.2f}, Çıktı Yaw (°): {output_yaw:.2f}, "
                f"Hata Pitch (px): {error_pitch_pixel}, Hata Pitch (°): {error_pitch_degree:.2f}, Çıktı Pitch (°): {output_pitch:.2f}")
            print(f"Gönderilen Orantılı Hareket: Delta Yaw: {output_yaw:.2f}, Delta Pitch: {output_pitch:.2f}")

            # Yeni orantılı hareket komutunu gönder
            self.send_proportional_move_command(output_yaw, output_pitch)
        else:
            print(
                f"DEBUG (process_tracking): Hedef deadband içinde veya min eşiğin altında. Hata Yaw: {error_yaw_pixel}px ({error_yaw_degree:.2f}°), Pitch: {error_pitch_pixel}px ({error_pitch_degree:.2f}°). Hareket komutu gönderilmedi.")
            self.target_info_label.setText(
    ...
    def process_tracking_to_home_position(self):
        """
        Taretin tanımlanmış bir 'ana' konuma dönmesini sağlar.
        ...
        new_pid_range_home = "HOME_POSITION"
        if self.current_pid_range != new_pid_range_home:
            print(
                f"DEBUG: PID aralığı değişti: {self.current_pid_range} -> {new_pid_range_home}. PID sıfırlanıyor (Ana Konum).")
            self.reset_pid_state()
        ...
        if output_yaw != 0.0 or output_pitch != 0.0:
            ...
            self._update_status_label(
                f"Durum: Ana konuma ulaşıldı: Yaw {current_yaw:.1f}°, Pitch {current_pitch:.1f}°")
            print("Ana konuma ulaşıldı.")
            return
...
if __name__ == '__main__':
    print("DEBUG: __main__ bloğuna girildi.")
    print("DEBUG: YOLO modeli ve global görüntü boyutları ayarlandı.")
    print("DEBUG: QApplication örneği oluşturulmaya çalışılıyor.")
    app = QApplication(sys.argv)
    print("DEBUG: QApplication örneği oluşturuldu.")
    window = HavaSavunmaArayuz()
    print("DEBUG: HavaSavunmaArayuz örneği oluşturuldu.")
    window.showMaximized()
    print("DEBUG: window.showMaximized() çağrıldı. QApplication olay döngüsü başlatılıyor.")
    try:
        sys.exit(app.exec_())
    except Exception as e:
        print(f"Uygulama beklenmedik bir hata ile kapandı: {e}")
        traceback.print_exc()
    print("DEBUG: QApplication olay döngüsünden çıkıldı.")
