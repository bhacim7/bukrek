...
yolo_model_task12_type = load_yolo_model(YOLO_MODEL_PATH)
yolo_model_task3_type = None

input_name = None
output_name = None

# Eğer model TensorRT ise, giriş/çıkış adlarını TensorRT motorundan al
if yolo_model_task12_type == "tensorrt":
    if trt_engine:
    ...
    yolo_model_task12 = yolo_model_task12_type
...
class HavaSavunmaArayuz(QWidget):
    def __init__(self):
        ...
        self.KI_YAW = 0.02  # 0.0001'den 0.0002'ye yükseltildi
        self.KD_YAW = 0.03  # 0.08'den 0.12'ye yükseltildi

        self.KP_PITCH = 0.4  # 0.04'ten 0.06'ya yükseltildi
        self.KI_PITCH = 0.005  # 0.0001'den 0.0002'ye yükseltildi
        self.KD_PITCH = 0.03  # 0.08'den 0.12'ye yükseltildi
        ...
        self.feedforward_yaw_gain = 0.5  # Devre dışı bırakmak için 0.0 olarak ayarla
        self.feedforward_pitch_gain = 0.5  # Devre dışı bırakmak için 0.0 olarak ayarla

        self.feedforward_velocity_threshold_px_s = 40.0
        ...
        self.yolo_ready = (yolo_model_task12_type is not None)
        self.model_is_tensorrt = (yolo_model_task12_type == "tensorrt")
        ...
        self.task3_start_button.clicked.connect(self.start_task3_engagement) # YENİ: Aşama 3 başlangıç butonu
    ...
    def cancel_task(self):
        ...
        self.current_tracked_target_bbox = None
        self.missing_frames = 0  # Sıfırla
        self.is_ready_to_engage_from_qr = False # YENİ: QR angajman durumunu sıfırla
        self.qr_degrees = {} # YENİ: Kaydedilen dereceleri sıfırla
        self.current_qr_char = None # YENİ: Mevcut QR karakterini sıfırla
    ...
    def start_task3_engagement(self):
        ...
        # Yeni YOLO modelini yükle
        global yolo_model_task3_type, yolo_model_task3
        if yolo_model_task3_type is None:
            yolo_model_task3_type = load_yolo_model(YOLO_MODEL_PATH_TASK3)
            if yolo_model_task3_type == "tensorrt":
                self.model_is_tensorrt = True
            else:
                self.model_is_tensorrt = False
                yolo_model_task3 = yolo_model_task3_type
    ...
    def update_frame(self):
        ...
        try:
            ...
            # Hangi YOLO modelinin kullanılacağına karar ver
            ...
            if self.is_target_active and current_yolo_model is not None:
                ...
                if self.active_task == 'task3' and self.waiting_for_new_engagement_command and not self.is_ready_to_engage_from_qr:
                    ...
                    data, _, _ = self.qr_detector.detectAndDecode(display_frame)
                    if data and data in self.qr_degrees:
                        ...
                        self._update_status_label(f"Durum: QR Kodu okundu: '{data}'. Hedef açıya dönülüyor: {target_yaw_from_qr}°")
                        self.target_info_label.setText(f"Hedef Bilgisi: QR Kodu ({data}) Okundu. Hedefe dönülüyor.")
                    ...
                    if self.active_task == 'task1':
                        ...
                        # Aşama 3: QR'dan alınan hedefi bul ve takip et
                        matching_detections = [d for d in detections if
                                               d['class_name'].startswith(self.current_tracked_target_class)]
                        ...
                        if candidate_target:
                            ...
                            detected_class_status = None
                            self._update_status_label("Durum: Angajman hedefi bekleniyor...")
            ...
            if self.active_task == 'task1':
                ...
                if current_target_bbox_for_pid and self.is_aimed_at_target and not self.target_destroyed:
                    ...
                    self._update_status_label("Durum: Aşama 3 - Hedef yok edildi. Yeni angajman bekleniyor.")
